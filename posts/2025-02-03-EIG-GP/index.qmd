...
We will also use the following notations to denote the various Gram matrices using kernel $k$: - $K = k(X,X)$, size $n \times n$.
...
| 3  | $K_*^{+T} (K^+ + \delta^2 I_{n+1})^{-1}$ | $O(m (n+1)^2)$ |
| 4  | $K_*^{+T} (K^+ + \delta^2 I_{n+1})^{-1} K_*^+$ | $O(m^2)$ |
| 5  | $\det\big(K_{**}^+ - K_*^{+T} (K^+ + \delta^2 I_{n+1})^{-1} K_*^+ \big)$ | $O({\color{red}m^3})$ |
...
$$
EIG(x) = \det(K_{**}^+) \cdot 1/ \det(K^+ + \sigma^2 I_{n+1}) \cdot \det \big(K^+ + \sigma^2 I_{n+1}- K_*^+ (K_{**}^+)^{-1}K_*^{+T} \big)
$$
...
| 8  | $1/ \det(K^+ + \sigma^2 I_{n+1}) \cdot \det \big(K^+ + \sigma^2 I_{n+1}- K_*^+ (K_{**}^+)^{-1}K_*^{+T} \big)$ | $O(1)$ |
