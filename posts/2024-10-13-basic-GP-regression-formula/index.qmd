...
| Operations        | Dimensions   | Cost  | Potential Tricks  |
|:-----:|:---:|:---:|:----------:|
|  Matrix Inversion    |  $\mathbb{R}^{n \times n}$ | $O(n^3)$  | Woodbury matrix identity |
|  Matrix Determinant    |  $\mathbb{R}^{n \times n}$ | $O(n^3)$  | Almost by-product of matrix inversion when using LU decomposition |
|  Matrix Multiplication    |  $\mathbb{R}^{n \times m}, \mathbb{R}^{m \times p}$ | $O(nmp)$  | / |
| Matrix Square Root (Cholesky)  |  $\mathbb{R}^{n \times n}$ | $O(n^3)$  | / |

Using those, we can realise the following computational costs of GP operations. 

| Operations |  Size     | Cost  |
|:-----:|:---------:|:---:|
|  Likelihood Evaluation    |  $n$ observations | $O(n^3)$  | 
|  Posterior Mean    |  $n$ observations, $m$ test points | $O(n^3 + n^2 m)$  |
|  Posterior Covariance    |  $n$ observations, $m$ test points | $O(n^3 + n^2 m + n m^2)$  | 
|  Posterior Sampling    |  $n$ observations, $m$ test points, $j$ samples | $O(n^3 + n^2 m + n m^2 + m^2j)$  |
