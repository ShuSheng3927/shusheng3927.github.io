...
We run MCMC on the constructed posterior using Python's [BlackJax](https://blackjax-devs.github.io/blackjax/) package [@cabezas2024blackjax]. For whatever reason, more involved MCMC algorithms like HMC and NUTS fail to produce reasonable chains (potentially due to poor selection of algorithm hyperparameters, or autograd not reacting well to our likelihood computation). Eventually, I turned to the basic RWM. After trying to hand-tune the stepsize (for each parameter) and getting frustrated, I used an adaptive version of RWM with Robins-Monroe adaptation [@andrieu2008tutorial] targeting the optimal acceptance rate (around 0.3, given the low posterior dimension [@roberts2001optimal]).

![(Top) Trace plots for sampled parameters with true values shown as red dashed lines. (Bottom) Histogram for the samples with true values shown as red dashed lines.](./mcmc_chain.png)
